...
/**
 * A singleton style manager to maintain Drivers to prevent test slowdown for
 * creating a browser for each class with tests.
 *
 * Also counts time to start a browser and extrapolates from that how much time
 * you have saved using such hacky code.
 */
public class Driver extends Thread {
	...
	public enum BrowserName {
		FIREFOX, GOOGLECHROME, SAUCELABS, IE, HTMLUNIT, GRID, FIREFOXPORTABLE, FIREFOXMARIONETTE, APPIUM, EDGE
	}
	...
	//public static String PROXYPORT = "4013";
	...
	public static WebDriver get() {
		...
		if (useThisDriver == null) {
			...
			// String defaultBrowser = System.getProperty(BROWSER_PROPERTY_NAME,
			// DEFAULT_BROWSER);
			...
			/*
			 * Note: I generally use the Java 1.7 format that you can see below this chunk
			 * of code. where the switch statement uses a String.
			 * 
			 * Java 1.6 does not support this, since people have tried to use this code on
			 * older versions of Java, and this is really the only Java 1.7 specific
			 ...
			 * 
			 * I could have used an enum, or refactored it to use objects but I wanted to
			...
			/*
			 * Code below requires Java 1.7 to switch on String This would be my default way
			 * of writing the code, but have not made it the default in this code base
			 * because people often compile against 1.6 settings.
			 */
			/*
			 * switch (defaultBrowser){ case "FIREFOX": useThisDriver = BrowserName.FIREFOX;
			 * break; case "CHROME": useThisDriver = BrowserName.GOOGLECHROME; break; case
			 * "IE": useThisDriver = BrowserName.IE; break; case "SAUCELABS": useThisDriver
			 * = BrowserName.SAUCELABS; break; case "HTMLUNIT": useThisDriver =
			 * BrowserName.HTMLUNIT; break; case "GRID": useThisDriver = BrowserName.GRID;
			 * break; default: throw new RuntimeException("Unknown Browser in " +
			 * BROWSER_PROPERTY_NAME + ": " + defaultBrowser); }
			 */
		...
		if (aDriver == null) {
			...
			// for WebDriver 3 compatibility I may need to set the FirefoxDriver to use the
			// legacy driver rather than marionette
			...
			case FIREFOXPORTABLE:
				...
				setDriverPropertyIfNecessary("seleniumsimplified.firefoxportable",
						"/src/main/resources/webdrivers//FirefoxPortable.exe", 
						"/src/main/resources/webdrivers/FirefoxPortable.exe");
				...
				aDriver = new FirefoxDriver(
						new FirefoxBinary(new File(System.getProperty("seleniumsimplified.firefoxportable"))),
						new FirefoxProfile());
				currentDriver = BrowserName.FIREFOX;
				break;
			...
			case FIREFOXMARIONETTE:
				...
				setDriverPropertyIfNecessary("webdriver.gecko.driver", 
						"/src/main/resources/webdrivers/geckodriver.exe",
						"/src/main/resources/webdrivers/geckodriver.exe");
				// setDriverPropertyIfNecessary("webdriver.gecko.driver",
				// "/usr/local/marionette/geckodriver-0.18.0-OSX",
				// "/usr/local/marionette/geckodriver-0.18.0-OSX");
				// private static String marionetteLocation =
				// "/usr/local/marionette/geckodriver-0.9.0-OSX";
			...
			case APPIUM:
				...
				appiumCapabilities.setCapability("deviceName",
						EnvironmentPropertyReader.getPropertyOrEnv("APPIUM_DEVICE_NAME", ""));
				appiumCapabilities.setCapability("platformName", Platform.ANDROID);
				appiumCapabilities.setCapability("app",
						EnvironmentPropertyReader.getPropertyOrEnv("APPIUM_BROWSER", "browser"));
			...
			case GRID:
				...
				String gridBrowserVersion = EnvironmentPropertyReader.getPropertyOrEnv("WEBDRIVER_GRID_BROWSER_VERSION",
						"");
				String gridBrowserPlatform = EnvironmentPropertyReader
						.getPropertyOrEnv("WEBDRIVER_GRID_BROWSER_PLATFORM", "");
				...
				try {
					// add url to environment variables to avoid releasing with source
					String gridURL = EnvironmentPropertyReader.getPropertyOrEnv("WEBDRIVER_GRID_URL",
	 ...
	 * in use.
	 * 
	 * I can still find out what currentDriver is with `Driver.currentDriver`. But
	 * if I want to know the browser, I should use `Driver.currentBrowser()`
	 * 
	 */
